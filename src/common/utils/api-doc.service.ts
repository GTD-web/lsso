...
export class ApiDocService {
    ...
    private RETRY_DELAY = 5000; // 2초
    private data: any;
    private dirPath = 'C:/Users/USER/Desktop/projects/sso-documents/docs/개발/03_api';

    async getApiJson(retries = this.MAX_RETRIES) {
        await new Promise((resolve) => setTimeout(resolve, this.RETRY_DELAY));
        try {
            const response = await axios.get('http://localhost:3030/api-docs-json');
    ...
    async generateApiDocs() {
        ...
        Object.keys(controllers).forEach((controller, index) => {
                ...
                if (metadata.parameters && metadata.parameters.length > 0) {
                    ...
                    if (pathParams.length > 0) {
                        ...
                        pathParams.forEach((param, index) => {
                            ...
                            markdownContent += `  "${param.name}": ${example} // ${required} ${
                                param.description || ''
                            }\n`;
            ...
            const docsPath = join(this.dirPath, `${index + 1 < 10 ? '0' : ''}${index + 1}_${controller}.md`);

            this.saveMarkdown(docsPath, markdownContent);
        });
    }

    private async saveMarkdown(filePath: string, content: string) {
